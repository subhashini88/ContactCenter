...
function PieReportData(init_data) {
    ...
    self.loadData = () => {
        ...
        self.data([data]);
        let layoutData = { title: '' };
        if (init_data.height) {
            layoutData.height = init_data.height - 2;
        };
        if (init_data.width) {
...
function BarGroupedReportData(init_data) {
    ...
    function _drillDownDefaultChart(data, divId) {
        ...
        let req = null;
        let type = "EXPIRATION";
        if (getTextValue(init_data.reportConfiguration().Name) === 'Upcoming contract terminations') {
            type = "TERMINATION";
        }
...
function SelectReportModal(init_data) {
    ...
    self.saveChart = (iItem) => {
        if (_validateSavedReportForm(iItem)) {
            lpservices.saveChartService(iItem, function (status) {
    ...
    self.deleteChart = (iItem) => {
        if (!iItem.isNew()) {
            lpservices.deleteChartService(iItem, function (resp, status) {
    ...
    function _loadMyReports() {
        ...
        lpservices.loadUserChartsService(function (charts, status) {
            if (status == SUCCESS) {
    ...
    function _loadSharedReports() {
        ...
        lpservices.getDefaultReportsService({}, (res, status) => {
...
function TabNode() {
    ...
    function _addSaveReportWarning(iItem) {
        ...
        if (unsavedDashboard) {
            ...
            $("#id_SaveReport_warning").text("Unsaved dashboards will be permanently deleted and cannot be restored.");
        }
    ...
    self.deleteReport = (iItem) => {
        ...
        if (unsavedTabs) {
            ...
            $("#id_DeleteReport_warning").text("Unsaved dashboards will be permanently deleted and cannot be restored.");
...
function ConfigurableReportModal() {
    ...
    self.populateCustAttribute = function () {
        lpservices.getCustomAttributesListService({}, function (data, status) {
            if (status === "SUCCESS") {
...
function loadGeneralCTRNameMap() {
    try {
        getGeneralCTRPreviewDetails((status, res) => {
            if (res) {
...
// const l_reportSelectModel = new SelectReportModal();
...
$(document).ready(() => {
    ...
    // ko.applyBindings(l_reportSelectModel, document.getElementById("div_editLayoutTileModal"));
