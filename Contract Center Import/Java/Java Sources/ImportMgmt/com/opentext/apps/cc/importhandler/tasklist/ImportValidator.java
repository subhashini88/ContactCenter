...
import com.opentext.apps.cc.importhandler.tasklist.ImportConstants.AssignmentType;
...
public class ImportValidator {
	...
	public ReportItem validate(Map<String, String> rowData, MetadataInitializer metadata, String jobId) {
		...
		if (!Utilities.isStringEmpty(rowData.get(ImportConstants.TASKLIST_LEGACYID)) && ((!Utilities.isStringEmpty(recordType) && !recordType.equalsIgnoreCase(ImportConstants.RecordType.TASK.getValue())) || Utilities.isStringEmpty(recordType))) {
	...
	public void validateTaskList(Map<String, String> rowData, ReportItem report) {
		if (Utilities.isStringEmpty(rowData.get(ImportConstants.TASKLIST_LEGACYID))) {
			createErrorMessage(ImportConstants.TASKLIST_LEGACYID, "Cannot be empty");
			...
			if (taskType.equalsIgnoreCase(TypeOfActivity.STANDARD.getValue())
				...
				if (!Utilities.isStringEmpty(state)) {
					if (Objects.isNull(metadata) || Objects.isNull(metadata.statesMap) || Objects.isNull(
							metadata.statesMap.get(state))) {
						String GCStateId = getGCStateId(state, metadata);
	...
	public String getGCStateId(String state, MetadataInitializer metadata) {
		int response = 0;
		int nodes[] = null;
		...
		try {
			...
			Object[] paramValues = { ImportConstants.PROCESS_NAME, "CTRACTLIST" };
			...
			if(Objects.nonNull(GCProcessId)) {
				...
				Object[] paramValues2 = { GCProcessId, "CTRACTLIST" };
